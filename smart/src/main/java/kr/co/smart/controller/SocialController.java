...
public class SocialController {
	...
	public String kakoLogin(HttpServletRequest request) {
		//인가 코드 받기
	...
	public String kakaoCallback(String code, HttpSession session) {
		if( code == null ) return "redirect:login";	// 카카오 로그인 실패시 로그인화면으로 연결
...
//			-H "Authorization: Bearer ${ACCESS_TOKEN}"
		...
		if( ! json.isEmpty() ) {
			...
			//id를 제외한 사용자 정보가 kakao_account에 있다
			json = json.getJSONObject("kakao_account");
			String email = common.hasKey(json, "email");
			String gender = common.hasKey(json, "gender", "female").equals("female") ? "여" : "남";
			...
			//카카오 프로필 정보를 사용자 정보로 관리하도록 MemberVO에 저장하기
		...
		return "redirect:/";	// 카카오 로그인 성공시 웰컴화면으로 연결
	}
...
//	}
	...
	public String naverCallback(String code, String state, HttpSession session) {
		...
		if (code == null || !state.equals(sessionState))
			return "redirect:/";
		...
		if (json.getString("resultcode").equals("00")) {
			...
			if (name.isEmpty()) name = common.hasKey(json, "name");
