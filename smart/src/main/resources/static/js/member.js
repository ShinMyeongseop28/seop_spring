...
function validStatus(){
	...
	$(".check-item").each(function(){
		...
		// 아이디는 사용가능이 아닌 경우 유효하지 않음
		if( $(this).is("[name=userid]") && ! desc.text().includes("사용가능")){
			valid = false;
		}
		if( ! desc.hasClass("text-success") ){
			valid = false;
...
var member = {
	...
	userpwStatus: function( pw, keyup ) {
		if( keyup ){ // 비번입력시 비번확인은 초기화하기
			$("[name=userpw_ck]").val("").closest(".input-check").find(".desc")
	...
	showStatus: function( tag, status ){
		...
		tag.closest(".input-check").find(".desc").text( status.desc )
								   .removeClass("text-success text-danger")
